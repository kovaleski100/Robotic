...
//////////////////////////////////////////////////
//// M̩todos SAMPLING, WEIGHTING e RESAMPLING ////
//////////////////////////////////////////////////
...
void MCL::sampling(int set, const Action &u)
{
    ...
    /// Para definir uma distribui̤̣o normal X de media M e variancia V, pode-se usar:
     //std::normal_distribution<double> samplerX(M,V);
    //std::normal_distribution<double> samplerX(0.2,0.5);
    /// Para gerar amostras segundo a distribuicao acima, usa-se:
     // = samplerX(*generator);
    /// onde *generator ̩ um gerador de numeros aleatorios (definido no construtor da classe)
    double alfa01 = 0.2;
    double alfa02 = 0.2;
    double alfa03 = 0.1;
    double alfa04 = 0.1;
    ...
    //Pose odometry = base.getOdometry();  seria odometry.theta em vez de particles[set][i].p.theta?



    for (int i=0; i < numParticles; i++)
    {
        std::normal_distribution<double> sampler01(0,alfa01*drot1 + alfa02*u.trans);
        std::normal_distribution<double> sampler02(0,alfa03*u.trans + alfa04*(drot1+drot2));
        std::normal_distribution<double> sampler03(0,alfa01*drot2 + alfa02*u.trans);

        double delta01 = drot1 - sampler01(*generator);
        double trans = u.trans - sampler02(*generator);
        double delta02 = drot2 - sampler03(*generator);

        particles[set][i].p.x = particles[set][i].p.x + trans*cos(particles[set][i].p.theta+delta01);
        particles[set][i].p.y = particles[set][i].p.y + trans*sin(particles[set][i].p.theta+delta01);
        particles[set][i].p.theta = particles[set][i].p.theta + drot1 + drot2;

    }

    /// Para acessar a i-̩sima particula, usar:
    // particles[set][i].p.x
    // particles[set][i].p.y
...
{
    /// TODO: fa̤a a pesagem de todas as particulas
    int N = numParticles;
    float sum = 0.0;
    if(set==0)
    {
        ...
        for(int i=0; i<N;i++)
        {
            ...
            {
                for(int k = 0; k<tanZ;k = k = k+ 10)
                {
                    zM = z[k];
                    zE = computeExpectedMeasurement(k, particles[set][i].p);
                    ex1 = exp(-0.5*(((zM-zE)*(zM-zE))/var));
                    ex2 = 1.0/sqrt(2*pi*var);
                    prob=prob*(ex1*ex2);
                }
                particles[set][i].w = prob;
                std::cout << prob <<std::endl;
                sum += particles[set][i].w;
            }
        }
        int knot = 0;
        sleep(knot);
        ...
        {
            if(sum != 0)
                particles[set][i].w = particles[set][i].w/sum;
            else
                particles[set][i].w = 1.0/N;
        }
    }else
    {
        for(int i=0; i<N;i++)
...
{
    // gere uma nova gera̤̣o de particulas com o mesmo tamanho do conjunto atual
    ...
    float MIN = 0;
    float MAX = 1.0/numParticles;
    /// Para gerar amostras de uma distribui̤̣o uniforme entre valores MIN e MAX, pode-se usar:
    ...
    int i = 0;
    float c = particles[set][0].w;
    float u = 0.0;
    int lasti = 0;
    for(int j=1; j<=numParticles; j++)
    {

        u = amostra + 1.0/numParticles*(j-1.0);
        ...
        if(i<numParticles)
        {
            nextGeneration[j-1] = particles[set][i];
            lasti = i;
        }
        else
            nextGeneration[j-1] = nextGeneration[lasti];
        std::cout<<i <<" " << numParticles<< " "<< j-1<<std::endl;
    }
...
/////////////////////////////////////////////////////
//// M̩todo Auxiliar para o Modelo de Observacao ////
/////////////////////////////////////////////////////
...
void MCL::initParticles(int set)
{
    ...
    for(int i=0; i<numParticles; i++){
        ...
        do{
            ...
            particles[set][i].p.theta = 0;//randomTh(*generator);
...
//////////////////////////////////////////////////////
//// M̩todo de Atualiza̤̣o da Media e Covariancia ////
//////////////////////////////////////////////////////
